...
| INV-8 | Logs/traces/metrics/crash/analytics contain no passwords, tokens, private keys, plaintext, contact book, full IP history, or raw recovery material. | **P → tested for the server** (2026-09-08). The promised log-redaction tests exist: `services/api/tests/log_redaction.rs` captures the REAL subscriber output while driving real traffic through the real router at `debug` verbosity, then asserts the password, access token, refresh token, username, account id, device id and the search term from the query string are all absent — with a guard that fails if the capture is empty, so it cannot pass vacuously. It found a genuine leak: `tokio_postgres` logs statement PARAMETERS at debug, which are search terms, profile fields and message ciphertext, so any operator setting `RUST_LOG=debug` was capturing exactly what this invariant forbids. Fixed by `redact::log_filter`, which appends non-overridable directives pinning those targets at `info`. Request logs are allow-listed (method, route SHAPE with ids collapsed and query string removed, status, latency, server-generated request id) and metrics carry NO labels, asserted in `metrics_endpoint.rs`. **Still P, not F:** this covers the server only — iOS/client-side logging and crash reporting are unverified, and the scrubber is a floor rather than a proof (it cannot catch every careless interpolation). |
