...
async fn request_lands_in_the_recipients_folder_then_accept_makes_friends() {
    ...
    let (status, req) =
        post_json_auth(&app, "/v1/message-requests", &a_tok, json!({ "account_id": b_acct })).await;
    ...
    assert_eq!(list[0]["conversation_id"].as_str().unwrap(), conversation_id);
    assert_eq!(list[0]["from"]["account_id"].as_str().unwrap(), a_acct);

    let (status, senders) = get_auth(&app, "/v1/message-requests", &a_tok).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(senders.as_array().unwrap().len(), 0, "the sender sees no incoming request");

    // A cannot pile on a second request to the same person.
    let (status, _) =
        post_json_auth(&app, "/v1/message-requests", &a_tok, json!({ "account_id": b_acct })).await;
    assert_eq!(status, StatusCode::CONFLICT, "one outstanding request per pair");
    ...
    assert_eq!(mine.as_array().unwrap().len(), 0, "accepted request leaves the folder");

    let (_status, friends) = get_auth(&app, "/v1/friends", &b_tok).await;
    assert!(
        friends.as_array().unwrap().iter().any(|f| f["account_id"] == a_acct),
...
async fn decline_with_block_stops_further_requests() {
    ...
    let (status, _) =
        post_json_auth(&app, "/v1/message-requests", &a_tok, json!({ "account_id": b_acct })).await;
    assert_eq!(status, StatusCode::FORBIDDEN, "a blocked sender cannot request again");
}
...
async fn requesting_a_friend_or_yourself_is_refused() {
    ...
    let (status, _) =
        post_json_auth(&app, "/v1/message-requests", &a_tok, json!({ "account_id": a_acct })).await;
