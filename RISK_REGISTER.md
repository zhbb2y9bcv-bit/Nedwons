...
| R-102 | Backend service (`services/relay`, DB, queues) not built; only `auth-core` pure logic is implemented and unit-tested. Postgres atomic challenge consumption and refresh rotation are specified but not yet exercised against a real DB. | High | OPEN | backend lead | In-memory stores prove the *logic*; the SQL implementations that enforce atomicity are Milestone 1/2 work. |
| R-103 | No Docker/containers on this machine; `infra/` deployment definitions are unvalidated locally. | Medium | OPEN | ops | Compose/K8s manifests can be written but not run here. |
