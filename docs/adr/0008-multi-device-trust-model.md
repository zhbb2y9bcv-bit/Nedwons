...
- **Status:** Accepted — **server-side ceremony implemented 2026-07-18** (enrollment, per-account cap, device list + owner-scoped revoke; migration V13 drops the single-active index). **Conversation participation for linked devices implemented 2026-09-09 (migration V27):** a device that joins the self-group is seeded into all of its account's conversations via the MLS **setup queue** — every routed member either holds the conversation's keys (`mls_added`) or awaits a Welcome that any set-up member's device delivers on its next sync, under an expiring claim so exactly one performs each add. The same queue powers multi-device group creation/adds (ALL of a member's devices, not just the primary), invite-link joins, approved join requests, and **deferred adds** (a member with no prekeys is queued, not failed, and admitted when they first open the app). Client half: `ConversationCoordinator.reconcileSetup()`. Supporting core change: members retain a 3-epoch message-secret window (`MAX_PAST_EPOCHS`, CRYPTOGRAPHY.md §2) so a sender one commit behind is not silently dropped. Remaining follow-ups: the on-device QR/SAS pairing UX, recovery-secret enrollment, transparency-logging of device events, blocking software-signers from approving (R-101/R-304), and cross-device outbound display merging (a message you sent from your phone renders as inbound on your tablet).
- **Date:** 2026-07-17
