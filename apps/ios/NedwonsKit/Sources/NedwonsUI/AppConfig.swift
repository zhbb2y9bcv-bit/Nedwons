...
/// - `NedwonsTransparencyLogKey` — the transparency log's public key (hex, X9.63), **pinned out of
...
///
/// An environment override (`NEDWONS_SERVER_URL`) wins over the plist for local runs.
public enum AppConfig {
    ...
    public static var serverURL: URL {
        if let env = ProcessInfo.processInfo.environment["NEDWONS_SERVER_URL"],
            let url = URL(string: env)
        {
            return url
        }
        if let s = infoString("NedwonsServerURL"), let url = URL(string: s) {
            return url
        }
        return devServerURL
    }
    ...
    public static var pinnedTransparencyLogKey: Data? {
        infoString("NedwonsTransparencyLogKey").flatMap { Hex.decode($0) }
    }
