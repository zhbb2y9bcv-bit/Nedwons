...
/// `SentinelClient` (HTTP) + `MlsClient` (MLS core). This is the app-facing extraction of the
...
public struct SelfGroupLinker: Sendable {
    ...
    public init(client: SentinelClient) { self.client = client }
    ...
    public func linkPendingSiblings(mls: MlsClient, accessToken: String) async throws -> LinkResult {
        ...
        for deviceID in pending {
            ...
            } catch let SentinelClient.ClientError.http(status, _) where status == 404 {
