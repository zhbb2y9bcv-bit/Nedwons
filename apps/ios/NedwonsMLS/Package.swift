...
// NedwonsMLS wraps the Rust `mls-core` (via the `mls-ffi` UniFFI boundary, ADR-0007) as a Swift
// package the iOS app links for on-device MLS. It is a SIBLING of NedwonsKit rather than a target
// inside it, so NedwonsKit's pure-Swift tests keep building/running with no native dependency; the
