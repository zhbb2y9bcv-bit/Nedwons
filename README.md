...
| `apps/ios/NedwonsApp` (composition layer: `AppComposition` + `ConversationCoordinator` — the messaging pipeline: prekey lobby, MLS group bootstrap, upload-with-retry, inbox receive loop — plus the real `SecretEngine` and self-group linker over the live core) | Swift 6.3.3 | ✅ `swift build` | ✅ `swift test` — 18 tests against the real Rust core, including the whole pipeline over an in-memory relay: DM round trip, 3-member growth commits, failed-upload retry delivered once, unsent survives relaunch, Welcome redeemed from a persisted lobby after relaunch, mute hold. The live run (`scripts/self_group_live_run.sh`) drives the same objects against the real relay → `LIVE_OK` |
