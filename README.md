...
| `apps/ios/NedwonsKit` (Swift crypto/protocol + full HTTP client + SwiftUI app: launch state machine, authentication, Chats/People/Settings, private aliases, group administration + moderation) | Xcode 26.6 / Swift 6.3.3 | ✅ `swift build` | ✅ `swift test` — 124 tests, including launch phases, session persistence, alias validation/encryption, search ordering, conversation-deletion semantics, and the group-admin surface (mute/unmute/mute-all/roles/add/remove) driven through the real model against an in-process fixture that applies the relay's rules |
| `apps/ios/NedwonsApp` (composition layer: `AppComposition` + `ConversationCoordinator` — the messaging pipeline: prekey lobby, MLS group bootstrap, upload-with-retry, inbox receive loop — plus the real `SecretEngine` and self-group linker over the live core) | Swift 6.3.3 | ✅ `swift build` | ✅ `swift test` — 22 tests against the real Rust core, including the whole pipeline over an in-memory relay: DM round trip, 3-member growth commits, adding to an existing group (MLS add + name hand-off), E2EE group rename, unread counts clearing on open, per-message time + sending state, failed-upload retry delivered once, unsent survives relaunch, Welcome redeemed from a persisted lobby after relaunch, mute hold. The live run (`scripts/self_group_live_run.sh`) drives the same objects against the real relay → `LIVE_OK` |
